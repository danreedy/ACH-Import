...
class Institution < ActiveRecord::Base
  ...
  def federal_reserve_bank?
    self.record_type_code.eql? 0.to_s
  end
  ...
  def routing_number
    self.record_type_code.eql?(2.to_s) ? self[:new_routing_number] : self[:routing_number]
  end
  ...
  def self.populate_database(options={})
    if Institution.count == 0
      ...
      directory.lines.each do |institution_data|
        parser = ACH::Parser.new(institution_data)
        institution = where(:routing_number => parser.routing_number).first
        unless institution.change_date.eql? parser.change_date
